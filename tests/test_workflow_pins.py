...
branch change what we execute after review.  That matters most in
``publish.yml``, which runs with ``id-token: write`` in the ``pypi``
environment, so a compromised action could push a release of this library.
"""
...
# `uses: owner/repo@ref` — ignores the trailing `# vX.Y.Z` version comment.
...
def _uses_steps(path: Path) -> list[tuple[str, str]]:
    ...
    for line in path.read_text().splitlines():
        ...
        if match:
            steps.append((match["action"], match["ref"]))
    return steps
